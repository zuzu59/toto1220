...
async function save() {
  ...
  try {
    ...
    saveError.value = error?.message || 'Enregistrement impossible'
  } finally {
    saving.value = false
  }
}
  ...
  <section class="page record-page">
    <div class="page-head">
      ...
      <div class="inline-actions">
        ...
        <button v-if="isEditing" class="primary-button" type="submit" form="record-form" :disabled="saving">
    ...
    <div v-else class="record-card">
      ...
      <template v-else>
        ...
        <form id="record-form" class="edit-grid" @submit.prevent="save">
      ...
      <p v-if="!state.unlocked && isEditing && hasSecrets" class="warning-text">
        L’application est verrouillée : les champs non secrets peuvent être modifiés sans déverrouiller. Les mots de passe existants seront conservés, mais vous ne pouvez pas les lire ni les modifier.
      </p>
    </div>
  </section>
</template>
